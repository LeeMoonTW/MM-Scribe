...
> ## ✅ 2026-09-11 實測:skill ID 配對與 TCP 重組,兩個舊 bug 一起解掉
>
> 樣本:Wireshark 實機錄包 5557 封包 / 289 筆傷害事件,場景是**三名治癒師同時對我
> 施放 1 技(生命連結)**,我攻擊時每人各觸發一道 3 HIT 的「光波」。
>
> **發現一:`0x4FEC` 與 `0x5235` 的前後順序不固定,取決於「誰放的」**
>
> | 情境 | 順序 | 筆數 |
> |---|---|---|
> | `SKILL.userId == DMG.userId`(自己動手打的) | DMG 在前、SKILL 在後 | **100 / 100** |
> | `SKILL.userId != DMG.userId`(連攜觸發) | **SKILL 在前、DMG 在後** | 184 / 189 |
>
> 零例外的二分(那 5 筆是配對工具在旗標撞號時挑錯邊,不是反例)。
> 舊的 `find_skill_id_after()` 只往後掃,碰到連攜傷害必然抓到**下一發**的 skill ID,
> 整批往後錯一格,最後一筆抓不到變 `??????`。§5.1 已改寫。
>
> **發現二:配對鍵不能用 attacker,要用旗標**
>
> 連攜時 `SKILL.userId` 是**放技能的隊友**,`DMG.userId` 是**被掛連結的自己**,兩邊本來
> 就不同 —— §5.1 原本建議的 attacker/target 比對用在一般攻擊上 100% 準,用在連攜上
> 100% 錯。實測可零歧義配對的鍵是 **`targetId` 相同 + 7 bytes 旗標全等**:289/289 全中,
> 0 筆落單。旗標在 `0x4FEC` 內位於 content+24(= magic+33),與 `0x5235` 的 content+32
> 是同一組值,連爆擊位元都同步。
>
> **發現三:TCP 未重組的漏包率遠比預期高**
>
> 289 筆傷害事件中 **16 筆(5.5%)** 被切在 TCP segment 邊界上,逐封包掃會整筆漏掉;
> `0x4FEC` 另有 6 筆。§2 原本寫「61 bytes 相對 MTU 很小,被切中機率低」——**與實測不符**,
> 因為連攜會讓單次爆發膨脹到 2000 bytes 以上。已加接續層,見 §2。
>
> **發現四:同一招會有多個 key1,差別是技能品質(符文變化)**
>
> 三名治癒師中,兩人的光波 key1 都是 `0x2298C38D`,第三人是 `0x1A19D6A4`
> —— **兩個不同的人共用同一個 key1,所以 key1 不是施法者身分**。已向使用者確認:
> 第三人的該技能帶符文變化。佐證:該施法者「帶追擊 bit 但沒有對應傷害飄字」的
> `0x4FEC` 是另外兩人的 3 倍(6 vs 2 vs 2)。
> ⚠ `skills.ini` 把這兩個 ID 分別命名為「召喚光靈」「生命連結」,實為同一招的兩種品質。
>
> **修正後端到端對照**(同一份錄包,重播進改好的解析器):
>
> | | 總傷害 | hit | `0x2298C38D` | `0x1A19D6A4` | 普攻 | `??????` |
> |---|---|---|---|---|---|---|
> | 改版前 | 643,550 | 220 | 310,861 / 86 | 200,550 / 59 | 91,025 / 46 | 41,114 / 29 |
> | 改版後 | **654,299** | **234** | **355,912 / 126** | **208,206 / 63** | 90,181 / 45 | **0** |
> | 重組真值 | 654,299 | 234 | 355,912 / 126 | 208,206 / 63 | 90,181 / 45 | 0 |
>
> 程式端新增 `(連攜)` 註記與黃字:`SKILL.userId != DMG.userId` 時標在技能名後。
> **連攜傷害不計入總傷害 / DPS / 技能排名**(那是隊友的技能),詳見 §5.1 末段。
...
**最後更新：2026-09-20** — 破防槽（暈眩值）完整解析，見 **§13.1**。
2026-09-12 — 連攜傷害的統計處置與「連攜攻擊偵測」選項（§5.1 末段）。
2026-09-11 — skill ID 配對規則與 TCP 接續層改寫（見檔頭第一個方塊與 §2 / §5.1）。
前一次大改為 2026-08-16 — 與第三方整理的 `packet-protocol.md`（版本 2026-08-15）對照後補全。
...
2. **事件實際總長為 `9 + size`，但程式刻意維持 `offset += 8 + size`**（2026-08-16 評估後決定不改）。
   ...
   - 因此保留 `+8` 這個「寧可早一格」的保守值。（`find_skill_id_after()` 已於 2026-09-11
     被 `find_dmg_skill()` 取代，改為以傷害事件本身為中心雙向掃，不再有「掃描起點」的問題。）
...
### TCP 位元組接續 ✅ 2026-09-11 已實作（原「已知漏包來源」）

協定要求依 `tcp_seq` 重組後再切包。舊版 `scapy` 是**逐封包**餵 `parse_payload()`，
被切在兩個 segment 之間的傷害事件會**整筆漏掉**（總傷害與 DPS 低估）。

**原本的評估錯了**：舊筆記寫「61 bytes 的事件相對 MTU 很小，被切中的機率低」，
2026-09-11 實測是 **289 筆傷害裡 16 筆被切到，5.5%**（`0x4FEC` 另有 6 筆）。
原因是連攜傷害會讓單次爆發膨脹到 2000 bytes 以上，遠超過單一 segment。

現在 `packet_callback` 改走 `_dmg_feed()`，**不是完整的 TCP stack**，只處理「連續」：

| 情境 | 處置 |
|---|---|
| `seq` 接得上上一包的尾巴 | 續在後面，被切兩半的事件補得回來 |
| 整包都落在目前緩衝區間內 | 重傳 → 丟掉 |
| 部分重疊 | 只接新的那一截 |
| 中間缺一段 / 亂序 | 先把還在等的事件放行，再從這包重來，且這包單獨判（退化成舊行為） |

- 重複計算靠 `stream["reported"]`（**絕對 seq 高水位**）擋掉，不靠內容比對 ——
  同一輪爆發出現兩筆數值與旗標完全相同的傷害是正常的（實測有 `751 / 751` 這種），
  內容比對會誤殺。
- **已知取捨**：亂序時排在高水位之前的那包會被整包跳過。少算一筆遠比重複算一筆好，
  而且實測 5557 封包一次亂序都沒發生。合成封包測試：切在事件中間 7/7、每 120 bytes
  切一刀 7/7、重傳 7/7、亂序 3/7（只保證不重複）。
- 配對要雙向掃（見 §5.1），所以事件**後方**也要收滿視窗才判得準；沒收滿就原地等
  下一包，逾時 `DMG_PAIR_FLUSH_SEC`（0.25 秒）強制放行。
- 緩衝每條連線保留 `DMG_STREAM_CARRY`（1024 bytes），須大於「視窗 400 × 2 + 事件 62」。
...
- ShowDamageFloater 的合法條件是 **`userId != targetId` 且兩者皆 ≠ 0**；`userId == targetId` 的對齊是 **decoy（誘餌），必須跳過**。
  ✅ 2026-09-09 已在 `parse_payload()` 加上這道守門（`attacker == target` 或任一為 0 就跳過）。
...
### 5.1 ✅ 配對規則（2026-09-11 實測重寫）

> 舊版這裡寫的是「往後掃 200 bytes 抓第一個 `0x4FC5`」，以及「建議改用 attacker/target
> 比對」。**兩句都是錯的**，已由實機錄包推翻，原文保留在 git 歷史。
...
樣本涵蓋自己的普攻、隊友的技能、怪物打人的攻擊 —— 只要 attacker 是施法者本人就是
「DMG 先」，零例外。連攜（治癒師 1 技光波之類）反過來。

**合理推測（非封包證據）**：自己動手時客戶端早就知道按了什麼，先給傷害數字即可；
被動觸發的要先告訴客戶端「誰放了什麼」才能播特效，所以 SKILL 排在前面。

因此**只能雙向掃**。單向往後掃會讓連攜傷害整批往後錯一格，最後一筆變 `??????`。

#### 配對鍵 = `targetId` 相同 + 7 bytes 旗標全等

**不能用 attacker**：連攜時 `SKILL.userId` 是放技能的隊友，`DMG.userId` 是被掛連結的
自己，兩邊本來就不同。attacker 比對用在一般攻擊上 100% 準，用在連攜上 100% 錯。

旗標位置：

| | 7 bytes 旗標起點 |
|---|---|
| `0x5235` | `magic+41` （= content+32） |
| `0x4FEC` | `magic+33` （= content+24） |

兩邊**完全相同**，連爆擊位元都同步。實測 **289/289 筆零歧義配對，0 筆落單**。

實作見 `find_dmg_skill()`：雙向掃 ±`DMG_SKILL_NEAR_WINDOW`（400 bytes，實測最遠 171），
只收 `targetId` + 旗標都吻合的候選，取距離最近的，同時回傳施法者 ID。

#### 一併解掉的兩個舊 bug

1. 舊 `find_skill_id_after()` 掃到 size ≠ 35 的 `0x4FEC` 會直接 `return None` 提早中止，
   不再往後找。
2. DoT 那幾筆本來就沒有伴生 `0x4FEC`（`b41 = 00`，不是攻擊動作），舊版會抓到**後面
   某筆別人攻擊事件**的技能 ID 污染排行。改成旗標比對後配不到就回 `None`，
   不列入技能排行 —— 這是正確行為。

#### 連攜傷害（`SKILL.userId != DMG.userId`）

傷害事件本身**只有兩個 ID：攻擊者（你）與目標**。189 筆連攜傷害的 53 bytes content
全部搜過，施法者 ID **一次都沒出現**（content 的 `20..31`、`41..52` 恆為 0，沒有藏欄位的空間）。
要知道是誰觸發的，唯一途徑是配對 `0x4FEC` 讀它的 `userId`。

#### 連攜傷害的統計處置（2026-09-12）

傷害事件的 attacker 欄雖然是自己（遊戲也算成你的輸出），但那是隊友的技能造成的，
**不該混進你的技能排名與 DPS**。設定裡新增「連攜攻擊偵測」（`[Tracking] detect_chain`，
**預設關閉**）控制要不要看得到它：

| | 選項關閉（預設） | 選項開啟 |
|---|---|---|
| 一般模式（已認出角色 ID） | **整筆剔除** —— 不計入統計、日誌不顯示、不進時間序列 | 日誌黃字標 `(連攜)`、**寫進時間序列（存檔留得住）**，但**仍不計入統計** |
| 強制偵測（`force_all`） | 黃字 + 計入統計 | 黃字 + 計入統計 |

強制偵測是旁路模式，連自己是誰都還沒認出來，分不出誰連攜誰 —— 那裡一律照收，
語意才跟「全部都收」一致。

> ⚠ **存檔會出現「事件總和 > 統計總傷害」**：選項開啟時，連攜事件進了 `damage_events`
> 卻沒進統計桶。所以時間序列的 flags 多了一個 `DMG_EVENT_CHAIN_BIT = 1 << 18`，
> 事後看存檔要靠它把連攜那幾筆篩掉，數字才對得起來。

切換選項只影響之後收到的封包，不會重算已累積的統計；中途切換會讓同一場的資料
前後定義不一致，要乾淨就按「清除」重來。
...
- [x] ~~**補上 §5.1 的 skill ID 配對驗證**~~ ✅ 2026-09-11 已實作（雙向 + `targetId`/旗標比對，289/289 全中）
...
- [x] ~~**TCP 依 seq 重組**再餵解析器~~ ✅ 2026-09-11 已加位元組接續層，見 §2
...
| skill ID 配對 | 抓最近的 0x4FC5，不比對 ID | attacker/target 應 match | ✅ 2026-09-11 改雙向 + 旗標比對；**protocol 的 attacker 建議對連攜傷害是錯的**，見 §5.1 |
...
| TCP 重組 | 無 | 必要（seq 銜接） | ✅ 2026-09-11 已加接續層（非完整 stack），漏包率實測 5.5%，見 §2 |
...
### 13.1 破防槽（暈眩值）— 解析完成（2026-09-20）

> 整段調查在 2026-09-20 一天內完成，共 16 輪實機錄包。以下是**結論**；
> 流水帳已刪，只保留會影響判斷的證據與踩過的坑。

#### A. 封包

三個 opcode，全部 `enc=0`、固定長度、開頭都是 `u64 entityId`（高 32 位恆 0，與傷害事件同一套 ID）：

| opcode | len | 版面 | 用途 |
|---|---|---|---|
| `0x1D595` | 13 | `[u64 eid][float32 值][1 byte]` | **破防槽當前值** |
| `0x1D596` | 21 | `[u64 eid][u32 blobLen=8][u64 值][1 byte]` | **敵人 HP** |
| `0x1D797` | 16 | `[u64 eid][u64 目標 eid]`，全 `FF` = 解除 | **鎖定目標** |

**最大破防槽不在上面任何一個** —— 它在**怪物登場包 `0x4E4C` 解壓後 content offset 203 的 u32**
（`MOB_BREAK_MAX_OFF`）。實測 `25 / 50 / 200` 分別對上 `1 / 2 / 8` 格 → **每格固定 25**。

`0x1D596` 的 `blobLen = 8` 不是 16 的倍數 → **沒有 protocol §6.2 說的 AES 加密**，直接就是數值。

#### B. 規則（全部實機確認）

- **是絕對值，不是百分比**。超過上限會被夾住 —— 鐵證：某招前兩次增量都是 `+25.894`，
  `180.096 + 25.894 = 205.990`，實際收到 **`200.000`**。木樁同樣驗到（`24.858 → 25.000`，Δ 只有 `+0.142`）。
- **封包不帶上限**，要靠登場包 @203；也可以用破防當下的峰值反推。
- **送的條件：你或隊友打到就送，與鎖定無關**。組隊時自己完全不出手、純看隊友打，照樣收得到。
  （早期曾誤判為「只送鎖定目標」，反例見 D 節。）
- **切換鎖定不會補送當前值**。切回一隻累積到 12.429 的木樁，3 秒內掃遍**所有** opcode
  都沒有任何封包帶那個值。鎖定當下伺服器確實會送 `0x1D5A3` / `0x1D697` / `0x1D5FD`，就是沒有破防槽。
  → 切目標後到下一次命中之前沒有新資料。
- **UI 沒顯示 ≠ 沒有封包**。木樁的 UI 不顯示破防槽，照樣送 `0x1D595`，滿值 25。
- **歸零有三種原因**：破防、自然衰退（沒人打一段時間）、死亡。
- **破防判定**：值達上限的**同一擊**，傷害事件會亮破防旗標；暈眩期間槽維持在上限，結束後歸 0。
- **13 bytes 版面最後那個 byte 恆為 `00`**，破防當下也沒變 → **不是「已破防」旗標**。
  要判斷有沒有破防仍然看傷害事件的 `b41 bit3` / `b57 bit0`。
- **破防增量不是招式固定值** —— 會被玩家的「破防數值」屬性影響。
  同一招在同一段時間內增量完全相等，是因為屬性沒變，不是招式常數。
  增量常成 4 倍關係（`0.298/1.191`、`0.259/1.036`、`0.388/1.554`），推測是招式係數的比例。
  → `增量 = f(招式係數, 玩家破防屬性[, 敵人?])`；玩家屬性在封包裡還沒找到，是否吃敵人屬性也未確認。

#### C. 程式實作

| 位置 | 內容 |
|---|---|
| `STATUS_OPS` / `_status_walk` / `_status_report` | 三個 opcode 的解析。守門同 `BUFF_OPS`：opcode + 長度完全相符 + `enc==0` + 高 32 位為 0 |
| `MOB_BREAK_MAX_OFF = 203` / `_gauge_take_max` | 登場包讀上限，依 eid 快取。值不是 25 的倍數時印 `⚠ 版面可能變了`，**不靜靜吃掉** |
| `_gauge_pair_delta` / `_gauge_take_delta` | 破防增量入列 → 傷害事件先進先出取用（見 D 節限制） |
| `_gauge_note` | **所有診斷輸出的閘門**：非開發者模式直接 return。資料路徑本身在發布版照跑 |
| `GAUGE_PROBE_RAW` | 未知 opcode 的 hex dump，預設 `False` |
| `GAUGE_MAX_HUNT` | 「25 整數倍」欄位廣掃，預設 `False`（已用它找到 @203） |
| `GAUGE_DELTA_HUNT` | 反查傷害事件有沒有夾帶增量，**結論已定為沒有**，可關 |
| `GAUGE_LOCK_DUMP_SEC` | 切鎖定後的窗口 dump，預設 `0`（已用它驗完「不補送」） |

開發者 LOG 的 `[Gauge]` 分類行：

```
⌗ <怪> 最大破防槽 50 (2 格)
破防槽 <怪> 19.356/50 (0.8/2 格) (Δ+0.259) 尾碼:00 | 距上次 0.2s | ⏱ 鎖定後 0.03s 首筆
⇄ <怪> 破防 Δ+0.259 ← 傷害 6,516 [技能名] | 早到 0.02s | ⚠ 佇列還有 N 筆,可能錯位
⊞ [技能名] 破防 +0.388 對 <怪> | 此招先前記到: +0.259(別隻怪)
⚔ <怪> 挨打 傷害:5,000 | 攻擊者:隊友 | 我鎖定中:<另一隻>
★ 破防旗標 <怪> 傷害:47,171 | 當下破防槽 200.000 | 本輪峰值 200.000
▼ <怪> 破防槽下降 200.000 → 0.000 | 本輪峰值 200.000 | ★ 達上限 200 → 破防
⌗ 配對統計:傷害 200 筆 | 破防增量入列 116 筆 | 配到 115 / 落空 84 | DoT免取 N | 間接 N | 追蹤中目標 8
HP <怪> 0 (Δ-515) 尾碼:00        ← 只在歸零時印 (STATUS_HP_LOG = "death")
```

**攻擊事件日誌**已改為四欄：`傷害 | 破防值 | 標籤 | 技能名稱`（破防值一位小數，配不到顯示 `-`）。
`_scaled_tab_stops` 同步改四個停靠點，前兩欄（數字）右對齊。

#### D. 已知限制（會影響顯示正確性）

1. **破防槽封包裡沒有攻擊者、沒有傷害值、連增量都沒有** —— 13 bytes 全滿，送的是絕對值，
   `Δ` 是我們自己相減算的。**傷害事件也不夾帶增量**（115 次掃描 × float32/u32/u16 含縮放
   × 整個事件長度，零命中）。所以「哪一擊造成多少」只能靠時間配對。
2. **配對是先進先出，不是精確歸因**。實機統計 `入列 116 / 配到 115`，幾乎零損耗，
   代表「破防槽比傷害事件早到」的順序假設成立（封包入口先跑 `_gauge_scan` 再跑 `_dmg_feed`，
   而傷害事件還要為了配技能 ID 多等一個視窗）。但同一隻怪在 0.5 秒內被多段或多人打到時，
   佇列裡誰對誰無從分辨。
3. **不是每筆傷害都有破防判定** —— DoT、部分間接傷害、某些技能的連段都不產生破防值。
   - `is_dot` 已排除（不取佇列，顯示 `-`）。
   - **間接傷害不能排除**：部分技能的間接傷害**會**產生破防值。排除的錯法和放任一樣糟 ——
     該取的沒取，Δ 留在佇列裡照樣被下一擊拿走。
   - 結果：凡是「不產生破防的非 DoT 傷害」都會偷走下一擊的 Δ，**破防值欄有雜訊**。
4. **隊友打的也會進同一個佇列**，無法分辨，會被自己的下一擊取走。

#### E. 下一步（未做）

- **找分辨旗標**（建議優先）：傷害事件的 `b43`/`b44`/`b46`/`b47` 還沒解，其中可能有
  「這一擊算不算破防判定」。現在每筆傷害都知道「有沒有配到 Δ」，直接拿這個布林值
  對那幾個位元組做關聯統計即可，跑一場副本應該就看得出來。
- 累積「哪些 skill_id 會產生破防」的清單 —— 但清單要靠有雜訊的配對結果建立，會互相污染，
  可靠度低於上一項。
- 找玩家的「破防數值」屬性（可能在 `0x5028` 角色資料，brotli 壓縮）。
- 確認增量是否也吃敵人屬性（同一招對不同怪給不同值）。`⊞` 行的目標名就是為此而標。
- 面板功能（破防槽進度條）：資料已齊備，`值 ÷ 25 = 格數`，不需要知道總格數。

#### F. 踩過的坑（給未來的自己）

- **診斷的統計行不能放在「只有成功時才會跑」的函式裡**。這個錯犯了兩次：
  「LOG 裡沒有那行」同時代表「沒找到」與「根本沒跑到」，是兩個相反的結論。
  統計要放在每次都會經過的路徑上，而且要分開計數才定位得到斷點。
- **`▼ 破防槽下降` 不等於破防**：怪死掉也會歸零，那種峰值只是「死的時候剛好累到多少」。
  現在靠 `峰值 >= 上限` 判定破防，HP=0 判定死亡（純看 HP 有時序缺陷：HP=0 的封包可能比
  破防槽歸零晚到）。
- **`_gauge_scan` 的 try/except 會吞掉一切**。測試用的假物件少一個方法或屬性時，
  錯誤被吞掉、該印的行沒印，**測試會假通過**。寫 smoke test 時不要手寫方法清單，
  改成自動抓取所有 `_gauge_/_status_` 開頭的方法，並補上 `is_dev_mode` /
  `ident_self_entity` 的類別層級預設。
  （本輪的 smoke test 寫在暫存目錄、未進版控 —— 需要時照這個原則重寫即可，
  §13.1 A／B 節的版面與實測值足夠合成封包。）
- **去重會蓋掉「切鎖定是否補送」這種測試**：值沒變就不印的機制，會讓「伺服器有送快照」
  看起來像「沒送」。測那種問題前要先清掉該目標的去重狀態。
- **廣掃要快取「掃過但沒找到」**，否則位置封包一秒數十則會拖垮攔截執行緒。
- **縮放整數比對要擋掉小值**：`Δ=0.388` 在 ×1 下四捨五入成 0/1，而封包裡到處都是 1，實測會誤報。
